...
const elements = {
    ...
    criticalErrors: document.getElementById('criticalErrors'),
...
function updateStats() {
    ...
    if (elements.criticalErrors) {
        elements.criticalErrors.textContent = totalErrors;
    }
...
function updateGroupTabs() {
    ...
    allTab.innerHTML = `全部 <span class="count">${Object.keys(currentReport.errors).length}</span>`;
    ...
    // 添加各个分组的标签
    const sortedGroups = Object.entries(currentReport.errors).sort(([keyA], [keyB]) => {
        if (keyA === '其他') return 1;
        if (keyB === '其他') return -1;
        return 0;
    });
...
function initBackToTop() {
    ...
    const scrollThreshold = 400; // 滚动阈值
