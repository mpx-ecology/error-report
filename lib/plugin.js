...
class WebpackErrorCollector {
  ...
  apply(compiler) {
    ...
    compiler.hooks.done.tap('WebpackErrorCollector', async (stats) => {
      ...
      try {
        ...
        if (processedErrors.length === 0 && processedWarnings.length === 0) {
        ...
        // const reportPath1 = path.join(outputDir, 'error-report1.json');
        fs.writeFileSync(reportPath, JSON.stringify(report, null, 2));
        // fs.writeFileSync(reportPath1, JSON.stringify(errors, null, 2));
  ...
  detectErrorType(error) {
    ...
    // 其他错误类型
    if (/\bTS\d{4,5}\b/.test(error.message)) return 'TSError';
    if (message.includes('[style compiler]') || /(Module\s+parse\s+failed|(?:Syntax|Parse)Error|(?:expected|unexpected|missing)\s+['"`\w\s]+|(?:undefined|unrecognized|unknown)\s+(?:variable|mixin|function|import)|failed\s+to\s+@?import|(?:stylus|sass|scss|less|css)\s+loader|(?:\.styl|\.sass|\.scss|\.less|\.css)\b|(?:Error:|ERROR\s+in)|(?:unclosed|unterminated|expected)\s+(?:selector|block|parenthesis|quotes?)|(?:Invalid\s+CSS|SassError|LessError)|(?:\$[\w-]+|@[\w-]+)\s+(?:not\s+found|undefined)|(?:\/\*[\s\S]*?\*\/)?\s*Line\s+\d+:\s+[^]+?(?:error|warning))/i.test(error.message)) return 'styleError';
    if (message.includes('[template compiler]') || message.includes('invalid expression'))return 'templateError';
    if (message.includes('[script compiler]') || /(?:Module\s+parse\s+failed|SyntaxError|Unexpected\s+token|Missing\s*[\]\}\)]|Unterminated\s+(?:string|regex)|(?:Expression|Statement)\s+expected|`[^`]*?\$\{[^}]*\}|=>|\.\.\.|class\s+[^{]*\{|extends\s+\w+|for\s*\([^;]*\bof\b|async\s+function|\byield\b|\bfunction\*\b|invalid\s+expression)/i.test(error.message)) return 'syntaxError';
    if (message.includes('[json compiler]')) return 'jsonError';
    if (message.includes('you may need an appropriate loader')) return 'LackOfLoader';
    if (message.includes('is not a page/component/static resource') || /(?:Module\s+not\s+found|Can't\s+resolve|Error:\s+ENOENT|Failed\s+to\s+compile|(?:import|require)\(['"](.*?)['"]\)|(?:Uncaught|Unhandled)\s+Error:\s+Loading\s+chunk|File\s+or\s+directory\s+not\s+found|Unable\s+to\s+locate\s+module|(?:Invalid|Malformed)\s+module\s+path|alias\s+not\s+configured|Cannot\s+find\s+(?:module|file)|\.(?:js|jsx|ts|tsx|vue|json|css|scss|less|styl|png|jpg|svg)\b.*?\b(?:not\s+found|missing|unresolved)|\[(?:mpx-loader|native-loader|plugin\s+loader)\]|Current\s.*\sregistered|Path\s.*\snot\s+a|The\s.*\spath\s.*\smust\s+be|资源\s.*\s通过分包|Get\s+extracted\s+file\s+error|conflict\s.*\soutputPath)/i.test(error.message)) return 'moduleNotFound';
    if (/(ESLint\s+[eE]rror|Failed\s+to\s+compile|(?:error|warning)\s+in\s+\.\/[^\s]+\s+\(line\s+\d+[^)]*\)|(?:no-[\w-]+|react\/[\w-]+|import\/[\w-]+)\s+[\w\s]+?eslint|(?:Parsing|Linting)\s+error|\.eslintrc(?:\.\w+)?\b|Unexpected\s+(?:token|identifier)|(?:error|warning):\s+[^\[]+?\[eslint\]|(?:module|rules)\s+not\s+found)/i.test(error.message)) return 'eslint';
    if (/(?:\[optionChain\]|loader options must be|Expected condition|grammar is not supported|only support one expression)/i.test(error.message)) return 'optionChainError';
    return 'unknown';
  }
