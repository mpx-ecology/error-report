...
class WebpackErrorCollector {
  ...
  detectErrorType(error, processObj) {
    ...
    if (message.includes('[mpx script error]') ||
        /(?:Module\s+parse\s+failed)/i.test(error.message)) {
      processObj['语法错误'] += 1
      return 'syntaxError'
    }
    ...
    if (/(?:Module\s+not\s+found)/i.test(error.message)) {
      processObj['资源加载错误'] += 1
    ...
    if (/Module\s+build\s+failed/.test(error.message)) {
      processObj['编译错误'] += 1
      return 'modulesBuildError';
    };
    processObj['未知错误'] += 1
    return 'unknown';
  }
